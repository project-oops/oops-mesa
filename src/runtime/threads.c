 ...
 *
 * Mesa touches `pthread_*` from exactly one file, its C11 threads layer, so this surface is
 * closed at 26 functions. Which 26, and what evidence stands behind each vendor twin, is
 * `docs/hardware/mesa-thread-surface-fw1240.md`; this file is that table turned into code.
 *
 * # Why the mapping is nearly one to one
 *
 * The platform's C library is FreeBSD-derived and every one of its thread types is a pointer:
 * `pthread_t`, `pthread_mutex_t`, `pthread_cond_t` and `pthread_mutexattr_t` are all pointers to
 * opaque structures, and the vendor calls take pointer-sized handles. So the handles pass
 * through unchanged and nothing here allocates, copies or wraps them. Two exceptions are handled
 * below: `pthread_key_t` is an `int`, and `pthread_once_t` is a structure rather than a pointer.
 *
 * # Two conventions that do not line up, and what is done about it
 *
 * **Arity.** Three vendor twins take a trailing name the POSIX form has no place for: create,
 * mutex init and cond init. A shim that delegated by a rule rather than per name would read a
 * register the caller never set. orbistoun hit exactly that and recorded it (its D385 and D475),
 * and oops-sdk's own declarations already carry the right arity. Each name here is written out.
 *
 * **Failure.** POSIX answers zero or an errno; the vendor answers `0x8002_0000 | errno`, a scheme
 * measured across five families and seven provoked failures (orbistoun D398, and obSCEne's
 * `posixerr` section relies on it). So a failure can be translated rather than guessed, which
 * matters most for `pthread_cond_timedwait`: Mesa's C11 layer turns `ETIMEDOUT` into a timeout
 * and anything else into an error, so a timeout reported as a generic failure would look like a
 * broken condition variable.
 */
 ...
 *
 * The first group is bound and working in oops-sdk today, and these declarations match its
 * `src/thread/thread.c` exactly rather than being re-derived. The second group is named
 * `present` in obSCEne's import census or in orbistoun's libkernel inventory but has never been
 * called from this collection, so its arity follows the POSIX form it mirrors and is marked as
 * the assumption it is. `REQ-20260914T1443Z-3ea7` on the obSCEne bus is the measurement that
 * would settle the whole group, including whether the portable names bind directly and make
...
__attribute__((weak)) int scePthreadCondTimedwait(void *cond, void *mutex, unsigned int usec);
...
/* Every object this shim creates carries a name, because the vendor calls take one and the
 * platform's own tools show it. One name for all of them would make a thread list useless, so
 ...
 *
 * Zero means success in both conventions, which is the only part that coincides. A vendor
 * failure carries the errno in its low bits under 0x8002_0000. Anything that does not match that
 * shape is reported as EINVAL rather than as a number invented here: a caller switching on
 * specific errno values then falls to its default branch instead of matching the wrong case.
 */
...
/* Nothing here can work if the platform did not bind the call. Reporting ENOSYS is honest and
 * makes the first use fail with a reason rather than dereferencing a null. */
#define NEED(fn) do { if (!(fn)) return ENOSYS; } while (0)
...
{
    /* This one answers a question rather than reporting a status, so it does not go through
     * to_errno: a non-zero result means equal. Falling back to a pointer comparison when the
     * vendor call is absent is safe, because the handles are pointers. */
...
void pthread_exit(void *value)
{
    ...
    /* The vendor call does not return. If it was not bound there is nothing sensible left to do
     * and returning would resume a thread that asked to stop, so this spins deliberately rather
     * than pretending the exit happened. */
    for (;;) { }
}
 ...
 *
 * POSIX takes an absolute deadline; the vendor call takes a relative timeout in microseconds. So
 * the deadline is converted against the clock, and a deadline already past becomes a zero wait
 * rather than a negative one, which would otherwise become an enormous unsigned timeout.
 */
...
/*
 * `pthread_once_t` is a structure here rather than a pointer, so unlike everything else in this
 * file it cannot be handed to the vendor call: the two layouts are not the same object and
 * passing one as the other would corrupt whatever sits after it.
 *
 * So this is the one thing implemented rather than delegated, over a single mutex. That is
 * heavier than the platform's own version and it is correct, which is the right trade for a call
 * that runs once per site for the life of the process.
 */
...
int pthread_once(pthread_once_t *once, void (*routine)(void))
{
    ...
    if (!s_once_lock_ready) {
        /* A race here would be a thread calling pthread_once before any other thread exists,
         * which is when Mesa does its first initialisation. If that assumption ever stops
         * holding, this needs the platform's own atomic initialisation and a request to go with
         * it. */
 ...
 *
 * `docs/hardware/mesa-thread-surface-fw1240.md` is the surface Mesa's C11 threads layer needs,
 * and it is closed at 26. These are the rest: read-write locks, barriers, condition-variable
 * attributes and two odds, referenced from Mesa outside that layer and reported missing by
 * `tools/what-is-still-needed.sh` once every archive was built.
 *
 * Evidence is thinner here and it is worth being exact about which. obSCEne's census records
 * `scePthreadRwlockRdlock`, `scePthreadRwlockWrlock`, the `scePthreadCondattr*` family and
 * `scePthreadGetcpuclockid` as present. It does not record the rwlock init, destroy and unlock
 * calls, nor any of the barrier family - while it does record `scePthreadRwlockattr*` and
 * `scePthreadBarrierattr*`, which is the same shape of hole the census showed for
 * `clock_gettime`: the attribute functions are there and the operations they configure are not,
 * which is a gap in the mine rather than a platform without barriers.
 *
 * So every twin below is declared weakly, as the ones above are. One that the platform does not
 * export resolves to nothing and its wrapper returns ENOSYS at the first call, with a name
 * attached, rather than the whole shim failing to link over a function Mesa may never reach.
 * `REQ-20260914T1743Z-2e08` asks for the whole group by name.
 */

__attribute__((weak)) int scePthreadRwlockInit(void *lock, const void *attr, const char *name);

 ...
 *
 * Some of Mesa's namespace-scope globals are **dynamically** initialised: the compiler cannot
 * fold their initialiser to a constant, so it leaves the object in `.bss` (zeroed) and emits a
 * constructor - registered in `.init_array` - that fills it at start-up. ACO's opcode table
 * `aco::instr_info` is one: `_GLOBAL__sub_I_aco_opcodes.cpp` copies the real per-generation
 * opcode numbers into it. GLSL's builtin-function and builtin-type tables are two more, and the
 * compiler's own `__cpu_indicator_init` (behind `__builtin_cpu_supports`) is a fourth.
 *
 * On an ordinary system the crt start-up files walk `.init_array` before `main`. A title here is
 * linked by oops-apps with its own entry point and no such files, so nothing was calling them and
 * every one of those globals stayed zero. The symptom that led here: ACO assembled every
 * instruction with opcode 0 - reading a zeroed `instr_info.opcode_gfx10[...]` - so a GFX10 shader
 * came out with GFX11 encodings and the GPU faulted ILLEGAL_INST decoding it (oops-mesa worklog
 * 062). The gfx_level was correct end to end; the table it indexed was empty.
 *
 * # Why it is here and not in the loader
 *
 * The module's own dynamic relocations are applied (function pointers and the GOT work, or nothing
 * would run), so the `.init_array` entries are correctly relocated - they were simply never
 * called. Rather than depend on whatever the platform's `ld-elf.so.1` does with `DT_INIT_ARRAY`
 * for a module in this shape, a title runs this itself, once, before it touches Mesa. That is the
 * same crt-start-up job `__dso_handle` above stands in for, kept in the same file for the same
 * reason.
 *
 * The bounds are the linker's encapsulation symbols, placed by the title's link script
 * (oops-apps `local_tls.ld`). They are declared weak so that a link which somehow omits the
 * section yields a null pair and an empty walk rather than an undefined-symbol error.
 */

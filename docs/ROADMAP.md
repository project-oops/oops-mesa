...
| 5 | the winsys, standalone (an ioctl with 22 command cases under libdrm, D005; the skeleton is in src/winsys and every command refuses loudly): allocate, map, submit, fence over oops-sdk's bindings, exercised by a title that submits the oracle record's stream and gets the oracle record's hash | the oops-sdk oracle record reproduced through the winsys on firmware 12.40 | |
...
| 7 | the static SDK: `dist/` with headers, archives and `oops-mesa.mk`; the `USE_MESA` switch in oops-apps' `app.mk`; an example title | a title in oops-apps builds with `make title`, packages with SELFish, deploys with Prosperous, and renders | |

/*
 * The runtime shim's host suite. Today that is one file: `src/runtime/stderr_to_klog.c`.
 *
 * # Why this is a separate binary from the winsys suite
 *
 * The thing under test *defines* `fprintf`, `fwrite`, `fputs`, `fputc`, `puts`, `fflush` and
 * `perror`. Linking it into the winsys suite would redirect that suite's own reporting into the
 * log sink and it would appear to pass silently. So it gets its own binary, and this file takes
 * care not to report through anything it has replaced.
 *
 * # Why it reports with write(2) rather than printf
 *
 * Because that care has to be real rather than a convention. clang rewrites `printf("literal\n")`
 * into `puts`, which this file's subject defines - so a reporter written with `printf` would be
 * swallowed by the code it is testing, intermittently, depending on whether a particular call had
 * a format specifier in it. `write` is not intercepted and cannot be rewritten into something that
 * is.
 *
 * That is not a hypothetical: the first version of `stderr_to_klog.c` intercepted two functions
 * because 281 source-level `fprintf` calls were counted, and the archives call seven different
 * things because of exactly this rewriting (worklog 036).
 */
#include <stdio.h>
#include <string.h>
#include <unistd.h>

/* The log sink `stderr_to_klog.c` writes to. Replaced here so the suite can see what came out. */
static int g_lines;
...
/* The subject, included rather than linked so that its statics are reachable and so that the
 * suite exercises the same translation unit a title compiles. */
...
int main(void)
{
    ...
    check(strcmp(g_seen[0], "assembled one character at a time") == 0, "with the text intact");
    ...
    check(g_lines == 1, "puts is captured, because stdout is no more readable than stderr");
    check(strcmp(g_seen[0], "a line on stdout") == 0, "and supplies its own line ending");
    ...
    /* Longer than a klog line: split, never truncated. The tail of a diagnostic is usually the
     * half worth having. */
    reset();
    char big[400];
